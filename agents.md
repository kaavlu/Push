...
- **Mock images:** Store under `assets/friends/`, `assets/groups/`, `assets/profile/`, `assets/onboarding/`; reference as path strings (e.g. `"assets/friends/chitty.png"`) and load via `PushImageAssets.image(named:)`.
...
- **Supabase row mapping:** PostgREST DTOs in `Push/Data/Supabase/Rows/` use snake_case properties matching table columns (no `CodingKeys`); map to domain via `*Row.*()` helpers. `timestamptz` columns decode as `String` and parse via ISO8601 (fractional seconds first) — not `Date`; default `JSONDecoder` date strategy fails on PostgREST strings. DB snake_case enums that don't match Swift `rawValue:` need explicit mapping — e.g. `profiles.availability_choice` (`free_now` → `FriendAvailabilityState`), `sharing_policies.audience_type` (`global_default` → `.globalDefault`). Day-1 profile toggle/connectors lists are synthesized empty client-side. `GroupMembershipRow` sets `sharingLevel: .full` (R3). Live reads: `SupabaseFriendRepository` selects `profiles` excluding self (RLS scopes friends/co-members; do not query `friendships`); `SupabaseGroupRepository` selects `groups` and `group_memberships`; `SupabaseSharingRepository` selects `sharing_policies`. Day-1 push/feed: `EmptyLivePushRepository` / `EmptyLiveFeedRepository` (`EmptyLiveRepositories.swift`) — empty reads, writes throw. All other live repo writes throw `SupabaseRepositoryError.writeNotSupported`.
- **Store refresh:** Mock: `InMemoryDatabase` bumps `@Published revision` via `didMutate()` after every write (never on reads). Live: no store — `onStoreChange`/`storeRevision` use a local subject (Day-1 no-op). ViewModels subscribe via `AppDataContainer.onStoreChange`, guard on `lastSeenRevision` (stamp at end of `load()` to avoid reload loops), and call `load()` on change; new mutating store methods must call `didMutate()`. Multi-row writes that must appear together (e.g. `createPush`, `updatePush` plan + responses) use a dedicated atomic store method with one `didMutate()`.
...
- **Friends screen:** `MainMapRoute.groups` presents `FriendsView` (bottom nav "Friends"; route id still `groups`). Friends mode: `FriendsViewModel` + `FriendsContentBuilder` — one row per direct friend via `VisiblePresence`; no visible presence → "Hidden right now" row (never dropped). Person rows use shared `FriendRowCard` on the cream surface (Friends list + group member list). Reuses `FriendPuckData`; friend tap uses the same `FriendDetailBottomSheet` → `FriendDetailSheet` stack as the map. Groups mode reuses `GroupsViewModel` + `GroupDetailView`. Cream page styling (`FriendsColor`, `friendsCard`), not map glass. DEBUG launch: `--friends`.
- **Onboarding Lab:** DEBUG-only 12-screen signup primer in `Push/OnboardingLab/`; `OnboardingLabViewModel` + screen splits (`*IntroScreens`, `*AuthScreens`, `OnboardingSignInScreen`, etc.), static `OnboardingLabFixtures` only — no repos/auth/backend in the lab. Parallel returning-user entry: welcome ↔ `.signIn` (`OnboardingAuthSwitchLink`). The theme + components (`OnboardingLabTheme`, `OnboardingLabComponents`) are promoted out of `#if DEBUG` for production reuse; the lab screens, fixtures, keypad flow, setup, and mock `completeSignIn()` stay `#if DEBUG`. DEBUG launch: `--onboardinglab`; jump to one screen with `--screen=<rawValue>` (e.g. `--onboardinglab --screen=signIn`).
- **Production auth:** `PushApp` → `RootView` (not `ContentView` directly); live unauthenticated users see `Push/Auth/AuthGateView`, which reuses the promoted onboarding theme/components (`OnboardingHeader`, `OnboardingCTAButton`, `OnboardingLab*` styling) wired to `AuthViewModel` + `AuthService`; `AuthFormModel` is the shared form-binding seam for production vs lab view models.
...
- **Profile:** `ProfileContentBuilder` (`Push/Data/Derived/`) derives `ProfileData` from `UserProfile`, `Person`, and self-scoped `VisiblePresence`; toggles/connectors load from canonical `UserProfile`. `ProfileViewModel` write-through: availability → `FriendRepository.setCurrentUserAvailability` (syncs `PresenceStatus` with `.manualOverride` + `UserProfile.chosenAvailability`); basics → `ProfileRepository.updateBasics` (displayName → `Person.firstName`; initials derive); privacy toggles → `updatePrivacy`. Place line uses vague label (`Near …`), not exact venue.
...
- **Tests:** After seed changes, run `DataLayerTests` (referential integrity). Live/mock isolation: `LiveContainerIsolationTests`. Auth bootstrap: `AuthBootstrapTests`. Map puck display: `MapRenderTests`. Layout tiers: `AdaptiveLayoutTests`. Supabase PostgREST mapping: `SupabaseMappingTests`. Full suite: `xcodebuild test -project Push.xcodeproj -scheme Push -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:PushTests -parallel-testing-enabled NO` — parallel testing intermittently drops the simulator runner in this environment; use an installed simulator name (Xcode 26: `iPhone 17`, not `iPhone 14`). SourceKit may false-positive on `Supabase` imports — trust `xcodebuild` (see `tasks/lessons.md`).
