...
- **Supabase row mapping:** PostgREST DTOs in `Push/Data/Supabase/Rows/` use snake_case properties matching table columns (no `CodingKeys`); map to domain via `*Row.*()` helpers. `profiles.availability_choice` is DB snake_case (`free_now`) — map explicitly to `FriendAvailabilityState`; `rawValue:` is camelCase and silently fails for multi-word states. Day-1 profile toggle/connectors lists are synthesized empty client-side. Live friend reads (`SupabaseFriendRepository`) select `profiles` excluding self — RLS scopes to friends/co-members; do not query `friendships` from the client. Live repo writes throw `SupabaseRepositoryError.writeNotSupported`.
