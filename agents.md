...
- **Store refresh:** `InMemoryDatabase` bumps `@Published revision` via `didMutate()` after every write (never on reads). ViewModels reload across screens via `AppDataContainer.onStoreChange`; new mutating store methods must call `didMutate()`.
