...
- **Production auth:** `PushApp` → `RootView`; live unauthenticated users see `AuthGateView` routing `AuthViewModel.screen` across welcome, signUp, signIn, checkEmail, forgotPassword, setNewPassword using promoted onboarding components. Email auth is fully live via `AuthService`/`AuthViewModel` only (no Supabase in Views): sign-up with name/handle (`SignUpResult` → session or check-email), sign-in, forgot/set password; user errors via `AuthUserMessage`. Reset links use `pushapp://auth/reset` — `RootView.onOpenURL` → `handleOpenURL`; recovery holds the gate until password update. Social providers are hidden, not stubbed. Fresh sign-in/restored session enter `.preparing` until snapshot warm (Retry/Sign Out on failure). `AuthFormModel` shared with the lab.
