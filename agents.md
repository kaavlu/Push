...
- **Target:** iOS 17+
- **Architecture:** MVVM
- **Data:** Parallel mock/live via `AppDataContainer` (`.mock` = `InMemoryDatabase` + `Local*` repos + seed; `.live` = Supabase repos + session identity). DEBUG defaults mock; `--live` opts in; Release always live. Day-1 live is **reads-only** social graph (profiles, friends, groups, sharing policies) — presence/push/feed return empty in live mode (no mock data leaks). Layout: `Push/Data/` — Domain, Seed, Store, Repositories, Derived, `AppDataContainer`. Spec: `tasks/spec.md` (Issue #27). Guides: `docs/data-architecture.md`, `docs/superpowers/specs/2026-07-05-data-architecture-design.md`, `docs/superpowers/plans/2026-07-12-supabase-migration-day1.md`. Supabase schema/migrations: `supabase/migrations/` (apply via MCP); use repo `.claude/skills/supabase*` skills. `SECURITY DEFINER` RLS helpers (e.g. `private.is_friend`) live in non-API-exposed `private` schema — not `public` — so policies can call them without PostgREST RPC exposure.
...
- **Mock by default.** DEBUG uses mock unless `--live`; Release uses Supabase. No direct Supabase/network from Views or application ViewModels — injected repos only; auth via `AuthService` (`AuthViewModel`). No real location.
...
Read: `CLAUDE.md` → `tasks/lessons.md` → `tasks/todo.md` → `git log --oneline -5`. Supabase/backend work: also read `tasks/spec.md` (Issue #27) and repo Supabase skills.
