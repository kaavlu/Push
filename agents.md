...
- **Diagnostics/logging:** Live startup and Supabase backend failures use `Push/Diagnostics/PushLog.swift` (`os.Logger` categories: `bootstrap`, `network`, `auth`). Wrap async backend calls with `PushLog.logged(_:)` — logs on failure, rethrows unchanged. **PII redaction:** never log an error's `.localizedDescription` or a `PostgrestError`'s `.message`/`.detail`/`.hint` (constraint violations can embed user input); use `PushLog.safeDescription(for:)` — error type + Postgres `.code` only (e.g. `23505`).
