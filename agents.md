...
- Live writes to social graph (friends/groups/sharing), realtime/subscriptions — profile self-writes (basics, toggles, availability), push coordination (create/edit/cancel/delete, RSVP), friend-request coordination (search/send/accept/deny via `0009`; remove via `0010`), and group creation + group-invite coordination (`create_group` / `resolve_group_invite` via `0011`) are allowed
...
- **Add Group:** 3-step full-screen `AddGroupFlowView` (name + optional session-only photo, 2–19 friend invitees, review) from Friends Groups mode “+”. `AddGroupViewModel` → `GroupRepository.createGroup`; picked photo is never uploaded — caller registers via `GroupsViewModel.registerSessionImage` before `GroupDetailView`. Cream Friends styling (no numbered step indicator). Live: `create_group` RPC (`0011`); invitees accept via Alerts.
