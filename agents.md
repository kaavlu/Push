...
- **Onboarding Lab:** DEBUG-only 12-screen signup primer in `Push/OnboardingLab/` (`#if DEBUG` throughout); `OnboardingLabViewModel` + screen splits (`*IntroScreens`, `*AuthScreens`, `OnboardingSignInScreen`, etc.), static `OnboardingLabFixtures` only — no repos, auth, permissions, or backend. Parallel returning-user entry: welcome ↔ `.signIn` (`OnboardingAuthSwitchLink`); sign-in (email/password or social) skips setup and lands on `.done`. Colors via thin `OnboardingLabColor` aliases over `PushColorPalette.Onboarding` and shared glass/control tokens. DEBUG launch: `--onboardinglab`; jump to one screen with `--screen=<rawValue>` (e.g. `--onboardinglab --screen=signIn`).
