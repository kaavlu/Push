...
- **Repositories:** All protocols are `async throws` (local impls never throw); ViewModels take repos via init (default from shared `AppDataContainer`) and expose primary content via `LoadState<Value>`. User-initiated edits persist via repo write methods (store + `didMutate()`), not ViewModel-local state only. Derived builders produce presentation structs from canonical domain data — presence builders use **`VisiblePresence`** (sharing-policy–filtered), never raw `PresenceStatus`. Views must not read mock enums or seed directly.
- **Supabase row mapping:** PostgREST DTOs in `Push/Data/Supabase/Rows/` use snake_case properties matching table columns (no `CodingKeys`); map to domain via `*Row.*()` helpers. `timestamptz` columns decode as `String` and parse via ISO8601 (fractional seconds first) — not `Date`; default `JSONDecoder` date strategy fails on PostgREST strings. `profiles.availability_choice` is DB snake_case (`free_now`) — map explicitly to `FriendAvailabilityState`; `rawValue:` is camelCase and silently fails for multi-word states. Day-1 profile toggle/connectors lists are synthesized empty client-side. `GroupMembershipRow` sets `sharingLevel: .full` (R3). Live reads: `SupabaseFriendRepository` selects `profiles` excluding self (RLS scopes friends/co-members; do not query `friendships`); `SupabaseGroupRepository` selects `groups` and `group_memberships`. Live repo writes throw `SupabaseRepositoryError.writeNotSupported`.
- **Store refresh:** `InMemoryDatabase` bumps `@Published revision` via `didMutate()` after every write (never on reads). ViewModels subscribe via `AppDataContainer.onStoreChange`, guard on `lastSeenRevision` (stamp at end of `load()` to avoid reload loops), and call `load()` on change; new mutating store methods must call `didMutate()`. Multi-row writes that must appear together (e.g. `createPush`, `updatePush` plan + responses) use a dedicated atomic store method with one `didMutate()`.
...
- **Onboarding Lab:** DEBUG-only 12-screen signup primer in `Push/OnboardingLab/`; `OnboardingLabViewModel` + screen splits (`*IntroScreens`, `*AuthScreens`, `OnboardingSignInScreen`, etc.). **Production auth** promotes shared theme/components + `OnboardingSignInScreen` / `OnboardingWelcomeScreen` out of `#if DEBUG`, wired to real Supabase via `AuthViewModel` + `AuthService` (Issue #27); lab-only fixtures, phone/keypad flow, 11-step setup, and mock `completeSignIn()` stay DEBUG-isolated. Lab uses static `OnboardingLabFixtures` only. DEBUG launch: `--onboardinglab`; jump to one screen with `--screen=<rawValue>` (e.g. `--onboardinglab --screen=signIn`).
...
- **Tests:** After seed changes, run `DataLayerTests` (referential integrity). Map puck display: `MapRenderTests`. Supabase PostgREST mapping: `SupabaseMappingTests`. Full suite: `xcodebuild test -project Push.xcodeproj -scheme Push -destination 'platform=iOS Simulator,name=iPhone 14' -only-testing:PushTests -parallel-testing-enabled NO` — parallel testing intermittently drops the simulator runner in this environment.
