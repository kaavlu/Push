...
- **Pushes tab:** `PlansView` splits owned vs invited pushes — `PlanData.isOwner`; `PlansViewModel.yourPushes` / `activePushes`. Review deck covers active (invited) pushes only. `PlanData.participants` is `[HangoutPerson]` (default `[]`) for owned-push avatar rows; seed non-empty for `isOwner` entries in `PlansMockData`.
