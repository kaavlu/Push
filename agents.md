...
- **Start Push flow:** `StartPushViewModel` loads groups/friends from `AppDataContainer` repos with `LoadState`; Step 4 suggestion buckets (`likelyFreeNow`, `mightBeInterested`) derive from canonical `PresenceStatus` availability (`.freeNow`/`.joinable` vs `.maybeDown`/`.freeSoon`) — never hardcode recipient IDs. Submit via `PushRepository.createPush(_:)` with `PushDraft` (`recipientIDs` are flow tokens `"group_<id>"` / `"friend_<id>"`); single selected group → `audience: .group` + `groupID`, otherwise `.inviteesOnly`; creator gets `.in`, deduped invitees `.pending` (creator excluded).
