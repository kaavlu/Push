//
//  PushReviewPlanCard.swift
//  Push
//
//  DS-025 — Review deck card (separate from Plans-glass family).
//  Shares status pill / person avatar subcomponents only.
//

import SwiftUI

/// Full-detail swipe-deck card on review glass. Not a Plans-glass preview card.
struct PushReviewPlanCard: View {
    let plan: PlanData
    ...
    var body: some View {
        ...
        .pushReviewDeckGlass(cornerRadius: ReviewPushCardLayout.cardCornerRadius)
    }
...
// MARK: - Review-local subviews (not cream list rows — DS-034)

struct ReviewSectionLabel: View {
    let title: String
    let trailing: String?
...
/// Migration shim — prefer `PushReviewPlanCard`.
typealias ReviewPushCard = PushReviewPlanCard
