...
- **Target:** iOS 17+
- **Architecture:** MVVM
- **Data:** Local in-memory store + async throwing repository protocols (mock backend; Supabase plugs in at repo layer later). See `AGENTS.md` and `docs/superpowers/specs/2026-07-05-data-architecture-design.md`.
- **Maps:** MapKit
...
Read: `CLAUDE.md` → `tasks/lessons.md` → `tasks/todo.md` → `git log --oneline -5`. When implementing a multi-task feature, also read the matching file under `docs/superpowers/specs/` (design) or `docs/superpowers/plans/` (execution).
